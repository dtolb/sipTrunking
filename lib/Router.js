...
var Router = function (id, token, secret, domId) {
	var userId = id || config.CATAPULT_USER_ID;
	var apiToken = token || config.CATAPULT_API_TOKEN;
	var apiSecret = secret || config.CATAPULT_API_SECRET;
	var domainId = domId || config.SIP_TRUNKING_DOMAIN_ID;
		...
		catapult.Domain.get(client, domainId, function (err, domain) {
			if (err) {
				...
				catapult.Application.create(client,
					{ "name"          : req.body.name,
					"incomingCallUrl" : config.NGROK_URL + "/calls/" + req.body.name },
					function (err, newApp) {
						if (err) {
							res.sendStatus(500);
						}
